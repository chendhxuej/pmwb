...
const addFormAgenda = () => {
  formAgendas.value.push({ seq: formAgendas.value.length + 1, topic: '', conclusion: '', division: '' })
}
...
const handleEdit = (row) => {
  ...
  formAgendas.value = (row.agendas || []).map((a, i) => ({ seq: i + 1, topic: a.topic || '', conclusion: a.conclusion || '', division: '' }))
  dialogVisible.value = true
}
