...
| 重点工作 | `/key-works` | ✅ | 总部试点/年度任务/每人一件事（专题工作）全周期：目标、验收标准、里程碑、分工、月/周计划、进展、成员待办、交付物（落 Obsidian `09-重点工作`） |
...
- Python 3.11+
- Node.js 18+
- MySQL 5.7+（需求催办表 `sent_emails` 由 Alembic 自动建表，新环境初始为空，不影响使用）
...
完整可移植的部署步骤（环境要求、建库、迁移、三服务启动、常见问题、隐私提示）见 **[`DEPLOY-MANUAL.md`](DEPLOY-MANUAL.md)**。

要点速览：

- 三个必需服务：主后端 `8000`、人员中台 `8001`、前端 `5173`，均依赖本地 `MySQL 3306`（两个库 `yxtyg_db` / `pmwb_master`）。
- 所有 `.env` 真实配置文件已被 `.gitignore` 排除，**仓库不含任何密码 / API Key**；配置均从 `.env.example` 复制填写。
- 统一邮件中心（3210）为独立外部服务，本仓库不含；缺失时邮件功能自动降级，不中断业务。
- 对外分享前请注意：`.workbuddy/memory/` 含个人工作日志与业务数据，建议按部署手册第 11 节处理后再公开。
