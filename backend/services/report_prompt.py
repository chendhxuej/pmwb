...
- key_work（重点工作）：total 总项数；by_category 分类分布（总部试点/年度任务/专题工作，已转中文）；by_status 状态分布（已转中文）；by_priority 优先级分布（已转中文）；active 为重点推进事项列表，**每个对象含 this_week_plan（本周计划 total/done/items，items 含 title/content/assignee/status/due_date）、next_week_plan（下周计划 total/items）、this_week_progress（本期进展日志列表，含 record_date/content/reporter）**，必须据此聚焦分析；completed_in_range 为本期完成事项列表；overdue 为逾期风险列表（计划完成日已过且未完结，含 title/owner/计划完成日/状态）。
...
一、本期概述（高度概括 + 核心盯办 + 人员时效）：这是全文导读，必须是对下方「二~七」核心结论的高度提炼，禁止与下方章节逐条重复数字。结构如下：
0. **报告标题（必须作为 H1 出现在文档最顶部）**：标题格式固定为 `# {type_label}（统计区间：YYYY-MM-DD ~ YYYY-MM-DD）`，例如 `# 工作周报（统计区间：2026-08-25 ~ 2026-08-29）`，其中 {type_label} 取输入数据中的对应中文标签。
1. 先用引用块（> ）给出「本期整体判断」一句总评（如：需求交付按计划、运营处置承压、协同落地滞后等综合判断）。
2. 「关键指标速览」表格（维度 / 本期指标两列，覆盖重点工作 / 需求 / 运营 / 会议 / 待办 / 知识六模块）。
3. **本期概述双段式（必须按以下两列并排结构输出，用 Markdown 表格或分块实现）**：
...
4. **核心盯办项（必须出现）**：从 key_work.overdue / key_work.active 中进度滞后或无本周进展的 P0/P1 / po_risk / operation_issue.high_sensitivity 未闭环 / todo.overdue_items / meeting_action.unfinished 里，挑选**最多 5 项**最关键的，用 bullet 列出「事项 + 负责人 + 必须本周完成/确认的动作」，让收件人一眼知道要盯什么。
5. **人员时效与改进要求（重点）**：基于 operation_issue.by_handler（各处理人 total/done/overdue/done_rate/overdue_rate），计算各相关人员的工单完成及时率，识别出**完成及时率偏低、超期突出的处理人**，对其**明确提出改进要求**（如：要求压缩 XX 工单处置时长、限期闭环超期项）；若存在多名问题人员，逐一点名与要求。若人员时效整体良好，简要肯定。
...
- ### 2.2 重点推进事项：逐条分析 active 中的重点工作，**必须基于每个对象的 this_week_plan 和 this_week_progress 描述本周计划完成情况**。详写原则：只对有本周计划/本周进展/逾期风险/P0/P1 的事项展开；无实质进展的（this_week_plan 为空且 this_week_progress 为空）用一句话概括即可，不要展开。**格式严格为**：【标题】（负责人：XXX，核心目标：XXX，进度：XX%，计划完成：YYYY-MM-DD）——本周计划完成情况/卡点——下周关键任务。最多详写 6 项，其余合并为「其他 X 项按计划推进」。
...
- ### 3.1 新增需求：本期新增跟踪需求（buckets.added），说明数量与重点方向；需求名超过 5 个时只列前 5，其余概括为「等 X 项」。
...
- ### 3.5 主动优化建议：基于 active_optimization 数据，概述本期新增/已采纳/不采纳/待评估的数量；对本期新增的优化建议，逐条列出「标题 / 业务管理员 / 关联需求 / 优化建议摘要」；超过 5 项时只列前 5。若数据为空，写「本期暂无主动优化建议」。
...
五、会议与协同：章节标题必须写作 `## 五、会议与协同`；开头用引用块 `> **本章概述**：...`（禁止作为二级标题）概括会议与协同整体情况。再展开：会议构成与结论提炼（非列标题）、行动项完成率与未闭环重点项（引用 meeting_action.unfinished 具体项与负责人）；会议数量超过 5 场时只列前 5 场关键结论，其余概括为「其他 X 场」。

六、个人待办：章节标题必须写作 `## 六、个人待办`；开头用引用块 `> **本章概述**：...`（禁止作为二级标题）概括个人待办整体执行状态。再展开：完成率 / 超期判断；超期项列 overdue_items 具体标题，超过 5 项时只列前 5，其余概括为「等 X 项」。
...
严格要求：① 一~八必须全部出现，且章节标题必须使用固定编号 `## 一、本期概述` / `## 二、重点工作` / `## 三、需求与交付` / `## 四、运营支撑` / `## 五、会议与协同` / `## 六、个人待办` / `## 七、知识中心` / `## 八、下期重点计划`；② 一（概述）必须包含报告标题（H1）、整体判断、关键指标速览表、双段式概述（Part A 工作成效 / Part B 待改进问题）、核心盯办项、人员时效与改进要求；③ 每个非概述章节开头的「本章概述」一律用引用块 `> **本章概述**：...` 呈现，**严禁**把「本章概述」写成 `## 本章概述` 二级标题；④ 二（重点工作）必须含「本章概述」+ 2.1~2.4 四个子章节，重点推进事项标注负责人与进度；⑤ 三（需求与交付）必须含「本章概述」+ 3.1~3.5 五个子章节，需求列表标注 SA；⑥ 四~七每章必须先「本章概述」再展开；⑦ 下期计划须按「重点工作 / 需求与交付 / 运营支撑 / 会议与协同 / 个人待办 / 知识中心」六个模块分别给出，禁止只给数字，必须列出具体对象（重点推进/逾期事项 / 高敏工单标题 / 未闭环行动项 / 超期待办标题 / 风险需求），对象清单见 key_work.active / key_work.overdue / high_sensitivity / meeting_action.unfinished / todo.overdue_items / po_risk / buckets.ongoing。⑧ **信息密度铁律**：每个子章节详细展开的具体对象不超过 5-6 条，其余一律概括为「其他 X 项按计划推进/已闭环」；禁止把无变化、无风险、无本周计划的对象写成大段文字。
...
_NEXT_PERIOD_REQ = {
    ...
    "weekly": "周报写「八、下周重点计划」：必须按「重点工作 / 需求与交付 / 运营支撑 / 会议与协同 / 个人待办 / 知识中心」六个模块分别给出下周计划。重点工作下周计划必须优先基于 key_work.active 中每个对象的 next_week_plan 逐项列出下周关键任务（不要泛泛而谈），并标注负责人与计划完成日；其他模块计划须对标本周进展（基于 key_work.overdue / po_risk、未闭环会议行动项、超期待办等推导）。",
