...
一、本期概述：用 2-4 句做整体总结——概括本期最关键的产出、进展、风险与总体判断，作为全文导读；必须给出「本期整体判断」（如推进顺利 / 交付偏慢 / 协同落地滞后）。
二、需求与交付：按 buckets 分桶叙述（交付类必须写「完成【需求】开发部署，核心实现【功能】，体现【价值】」）。必出分析维度——交付节奏判断（是否按期 / 后移）、存量进行中需求规模；单列「PO 级交付风险」列清单并提示卡点。
三、运营支撑：必出分析维度——① 工单高发类别及是否需要专项治理；② 高敏（P0/P1）工单盯办结果；③ 处理人时效（谁积压、谁超期，处置瓶颈）。不得只贴原始统计。
四、会议与协同：必出分析维度——提炼各会议达成的结论/决议（非列标题）、会议主题构成；单列「会议行动项完成率」并点出未闭环重点项与落地卡点。
五、个人待办：必出分析维度——个人完成率/超期判断，是否需优先清理；不与他人混淆。
六、知识中心：说明维护情况（条数/分类构成），给出知识沉淀活跃度判断；若为空注明「本期暂无」。
七、下期重点计划：{next_req}

严格要求：① 一~七必须全部出现，缺模块保留标题并注明「本期暂无」，严禁省略任一模块；② 下期计划须按「需求与交付 / 运营支撑 / 会议与协同 / 个人待办 / 知识中心」五个模块分别给出计划，对标本期进展推导；③ 每个模块必须有「数据 + 一句判断/结论」，禁止纯罗列。
