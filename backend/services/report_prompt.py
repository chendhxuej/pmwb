...
一、本期概述（高度概括）：这是全文导读，必须是对下方「二~七」核心结论的高度提炼，禁止与下方章节逐条重复数字。结构如下：
...
2. **本期概述双段式（必须按以下两列并排结构输出，用 Markdown 表格或分块实现）**：
...
3. **核心盯办项（必须出现）**：从 key_work.overdue / key_work.active 中进度滞后或无本周进展的 P0/P1 / po_risk / operation_issue.high_sensitivity 未闭环 / todo.overdue_items / meeting_action.unfinished 里，挑选**最多 5 项**最关键的，用 bullet 列出「事项 + 负责人 + 必须本周完成/确认的动作」。
4. **人员时效与改进要求**：基于 operation_issue.by_handler，点名完成及时率偏低的处理人并提出改进要求。
...
四、运营支撑：章节标题必须写作 `## 四、运营支撑`；开头用引用块 `> **本章概述**：...`（禁止作为二级标题）概括运营工单整体态势（总量 / 高发类别 / 时效一句话）。再展开：① 类别分布表；② 高敏（P0/P1）工单盯办：只列**未闭环/有升级风险**的项，已关闭的简写为「X 项已闭环」；③ 处理人时效与改进要求（同概述点名机制）。**注意**：不要单独列出"周期外在途跟踪"章节，所有工单统一分析，已挂起/已暂停的工单不计入统计。
...
严格要求：① 一~八必须全部出现，且章节标题必须使用固定编号 `## 一、本期概述` / `## 二、重点工作` / `## 三、需求与交付` / `## 四、运营支撑` / `## 五、会议与协同` / `## 六、个人待办` / `## 七、知识中心` / `## 八、下期重点计划`；② 一（概述）必须包含报告标题（H1）、整体判断、双段式概述（Part A 工作成效 / Part B 待改进问题）、核心盯办项；③ 每个非概述章节开头的「本章概述」一律用引用块 `> **本章概述**：...` 呈现，**严禁**把「本章概述」写成 `## 本章概述` 二级标题；④ 二（重点工作）必须含「本章概述」+ 2.1~2.3 三个子章节（重点推进事项 / 本期完成 / 风险与逾期），重点推进事项标注负责人与进度；⑤ 三（需求与交付）必须含「本章概述」+ 3.1~3.5 五个子章节，需求列表标注 SA，其中 3.5 须标注为内部主动建议需业务评估；⑥ 四~七每章必须先「本章概述」再展开；⑦ 下期计划须按「重点工作 / 需求与交付 / 运营支撑 / 会议与协同 / 个人待办 / 知识中心」六个模块分别给出，禁止只给数字，必须列出具体对象；⑧ **信息密度铁律**：每个子章节详细展开的具体对象不超过 5-6 条，其余一律概括为「其他 X 项按计划推进/已闭环」；⑨ **强制完整性**：一~八所有章节都必须输出，不得省略；⑩ **数据过滤要求**：需求、运营工单、重点工作中，已挂起/已暂停/暂停状态的项不计入统计和列表。
