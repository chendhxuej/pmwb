...
- **报告标题（必须作为 H1 出现在文档最顶部，独占一行）**：标题格式固定为 `# {type_label}（统计区间：YYYY-MM-DD ~ YYYY-MM-DD）`，例如 `# 工作周报（统计区间：2026-08-25 ~ 2026-08-29）`，其中 {type_label} 取输入数据中的对应中文标签。
...
四之壹、一线调研：章节标题必须严格写作 `## 四、一线调研`；开头用引用块 `> **本章概述**：...`（禁止作为二级标题）概括本期调研工单整体态势（总量 / 子类构成 / 地市分布 / 高影响未闭环项一句话）。再展开：① 子类分布表（领导调研 / 一线驻点）；② 地市分布；③ 高影响（P0/P1）未闭环工单盯办（引用 research_issue.high_impact，列出 issue_no/title/city/impact_level/负责人）。若数据为空，本章写「本期暂无一线调研工单」但仍保留章节标题和概述。
...
八、下期重点计划：{next_req}（章节标题必须写作 `## 九、下期重点计划`）
...
严格要求：① 一~九必须全部出现，且章节标题必须使用固定编号 `## 一、本期概述` / `## 二、重点工作` / `## 三、需求与交付` / `## 四、运营支撑` / `## 五、一线调研` / `## 六、会议与协同` / `## 七、个人待办` / `## 八、知识中心` / `## 九、下期重点计划`；② **文档最顶部必须是 H1 报告标题**（格式 `# 工作周报（统计区间：YYYY-MM-DD ~ YYYY-MM-DD）`），H1 下方紧接着引用块（本期整体判断）和概述内容，**不要单独输出"一、本期概述"作为 H2 标题**；③ 每个非概述章节开头的「本章概述」一律用引用块 `> **本章概述**：...` 呈现，**严禁**把「本章概述」写成 `## 本章概述` 二级标题；④ 二（重点工作）必须含「本章概述」+ 2.1~2.3 三个子章节（重点推进事项 / 本期完成 / 风险与逾期），重点推进事项标注负责人与进度；⑤ 三（需求与交付）必须含「本章概述」+ 3.1~3.5 五个子章节，需求列表标注 SA，其中 3.5 须标注为内部主动建议需业务评估；⑥ 四~八每章必须先「本章概述」再展开；⑦ 下期计划须按「重点工作 / 需求与交付 / 运营支撑 / 一线调研 / 会议与协同 / 个人待办 / 知识中心」七个模块分别给出，禁止只给数字，必须列出具体对象；⑧ **信息密度铁律**：每个子章节详细展开的具体对象不超过 5-6 条，其余一律概括为「其他 X 项按计划推进/已闭环」；⑨ **强制完整性**：一~九所有章节都必须输出，不得省略；⑩ **数据过滤要求**：需求、运营工单、重点工作中，已挂起/已暂停/暂停状态的项不计入统计和列表。
