...
- operation_issue（运营支撑）：by_category 各子类工单量（已转中文，**注意：不含一线调研/领导调研类，此类已独家归口 research_issue**）；by_status 状态分布（已转中文）；by_impact 影响等级分布（已转中文）；by_handler 各处理人（含 total 总量/done 已办/overdue 超期/done_rate 完成率/overdue_rate 超期率）；high_sensitivity 为运营侧高敏（P0/P1）工单清单（含 issue_no/title/category/status/handler/impact，均已转中文）。
- research_issue（一线调研/领导调研，独家归口第四章 4.4）：items 为本期调研工单明细（含 issue_no/title/sub_type/status/city/impact_level/situation_desc/feedback_deadline/business_admin）；by_sub_type 子类分布（已转中文）；by_status 状态分布（已转中文）；by_city 地市分布；high_impact 为高影响（P0/P1）未闭环工单清单。运营工单中的「领导调研」类已在此处统一归口，勿在运营支撑的其他子章节重复列示。
...
一、本期概述（执行摘要 · 全文导读）：这是给领导看的「一页纸结论」，**必须只给判断与行动指向，禁止罗列二~七章的具体数字与明细表格**。**注意：以下所有内容都必须放在 H1 标题的下方，不要作为独立章节输出**。结构如下：
- **报告标题（必须作为 H1 出现在文档最顶部，独占一行）**：标题格式固定为 `# {type_label}（统计区间：YYYY-MM-DD ~ YYYY-MM-DD）`，例如 `# 商客市场能力建设与运营工作周报（统计区间：2026-08-25 ~ 2026-08-29）`，其中 {type_label} 取输入数据中的对应中文标签，**必须原样完整使用、禁止简写或改写**。
- **本期整体判断（引用块 > ）**：一句话总评，必须给出明确「状态定调」（如「交付失速、风险聚集」「稳中有忧、局部承压」），**禁止堆砌「X平稳/Y承压/Z滞后」式形容词串**；格式：`> **本期整体判断**：<一句话定调 + 最关键依据>`。
- **核心定调（3 条判断，非数据）**：用 3 个 bullet 给出本期最关键的结论性判断，每条格式「<模块>：<结论 + 因→果>」，例如「交付侧：3 项 P0 需求零上线计划，交付节奏已实质失速」。禁止写具体数字（数字留待各章）。
- **本周必须攻坚 TOP3（最多 3 项）**：从 key_work.overdue / operation_issue.high_sensitivity / requirement.po_risk / todo.overdue_items 中挑最关键的 P0/P1，每条「【优先级】事项（负责人）——本周必须<具体动作>」。**禁止超过 3 项**，且不得与下方章节逐条重复（只做行动指向）。
- **处置时效预警（一句话）**：只点名完成率偏低的处理人，并写明「详见第四章 4.3 处理人时效与改进要求」，**不列表、不重复数字**。

**概述章节铁律**：① 总字数 ≤ 600 字；② 禁止出现任何明细数字表格（Part A/Part B 数字表一律删除）；③ 与二~七章零数字重复——本摘要只做「判断 + 行动指向」，所有量化数据下沉到对应章节。
...
四、运营支撑：章节标题必须写作 `## 四、运营支撑`；开头用引用块 `> **本章概述**：...`（禁止作为二级标题）概括运营工单整体态势（故障/异常类总量 + 高发类别 + P0 状态一句话，**注意运营工单不含一线调研/领导调研，调研类已独家归口 4.4**）。再**严格按固定编号**展开以下 4 个子章节（顺序即优先级，高敏置顶）：
- **### 4.1 高敏（P0/P1）工单盯办**（置顶，最优先）：只列**未闭环 / 有升级风险**的项，已闭环的集中一行「✅ 已闭环 X 项（清单略）」；每条给出「工单编号 / 标题 / 类别 / 影响 / 处理人 / 状态 / 本周盯办要求」，未闭环 P0 必须给具体动作与时限。
- **### 4.2 工单类别与趋势**：① 类别分布表（故障类：BUG管理/数据异常管理/热点投诉/主动运营分析/临时交办任务）；② **趋势判断**（如 BUG 占比偏高→系统性稳定性问题、同类问题高频复发→根因治理）；③ 提示重复/高频问题聚类，指向根因而非现象。
- **### 4.3 处理人时效与改进要求**：基于 by_handler，点名完成率偏低/超期的处理人，表格列出「处理人 / 总量 / 已办 / 完成率 / 改进要求（动作+时限）」——这是本章最 actionable 的部分，须完整列出，不要只点名不给药。
- **### 4.4 一线调研**：基于 research_issue 数据，用三级标题单独成节——先一句话概括（总量 / 子类构成「领导调研·一线驻点」/ 地市分布），再列工单明细（issue_no / 标题 / 地市 / 状态 / 业务管理员）与高影响（P0/P1）未闭环盯办；若数据为空写「本期暂无一线调研工单」但保留三级标题。**一线调研不计入运营工单总量，勿在 4.1~4.3 重复列示**。

**运营支撑铁律**：① 4.1~4.4 固定编号与顺序不可省略、不可调换；② 高敏表置顶且只列未闭环；③ 一线调研独家在 4.4，运营总量与高敏表均不含调研类；④ 已挂起/已暂停工单不计入统计。
...
严格要求：① 一~八必须全部出现，且章节标题必须使用固定编号 `## 一、本期概述` / `## 二、重点工作` / `## 三、需求与交付` / `## 四、运营支撑` / `## 五、会议与协同` / `## 六、个人待办` / `## 七、知识中心` / `## 八、下期重点计划`；② **文档最顶部必须是 H1 报告标题**（格式 `# 商客市场能力建设与运营工作周报（统计区间：YYYY-MM-DD ~ YYYY-MM-DD）`，须原样使用完整业务标签），H1 下方紧接着引用块（本期整体判断）和概述内容，**不要单独输出"一、本期概述"作为 H2 标题**；③ 每个非概述章节开头的「本章概述」一律用引用块 `> **本章概述**：...` 呈现，**严禁**把「本章概述」写成 `## 本章概述` 二级标题；④ 二（重点工作）必须含「本章概述」+ 2.1~2.3 三个子章节（重点推进事项 / 本期完成 / 风险与逾期），重点推进事项标注负责人与进度；⑤ 三（需求与交付）必须含「本章概述」+ 3.1~3.5 五个子章节，需求列表标注 SA，其中 3.5 须标注为内部主动建议需业务评估；⑥ 四（运营支撑）必须含「本章概述」+ **4.1~4.4 四个固定编号子章节（4.1 高敏盯办 / 4.2 类别与趋势 / 4.3 处理人时效 / 4.4 一线调研），顺序不可调换、高敏置顶**；一（本期概述）只含整体判断 + 核心定调 + TOP3 盯办 + 时效预警，**禁止 Part A/Part B 数字表**；五~七每章必须先「本章概述」再展开；⑦ 下期计划须按「重点工作 / 需求与交付 / 运营支撑（含一线调研）/ 会议与协同 / 个人待办 / 知识中心」六个模块分别给出，禁止只给数字，必须列出具体对象；⑧ **信息密度铁律**：每个子章节详细展开的具体对象不超过 5-6 条，其余一律概括为「其他 X 项按计划推进/已闭环」；⑨ **强制完整性**：一~八所有章节都必须输出，不得省略；⑩ **数据过滤要求**：需求、运营工单、重点工作中，已挂起/已暂停/暂停状态的项不计入统计和列表；⑪ **跨章去重铁律**：同一具体对象只在最相关的一章详列，其余章节仅「点名 + 指向」——人员时效唯一归属 4.3（一、本期概述仅一句话预警、不列表）；核心盯办 TOP3 只在「一、本期概述」出现，不重复于二~七章；八为完整下期计划，一~七不展开下期动作；一线调研独家在 4.4，运营总量与高敏表均不含调研类。
