...
3. **核心盯办项（必须出现）**：从 key_work.overdue / key_work.active 中进度滞后或无本周进展的 P0/P1 / po_risk / operation_issue.high_sensitivity 未闭环 / todo.overdue_items / meeting_action.unfinished 里，挑选**最多 5 项**最关键的，用 bullet 列出「事项 + 负责人 + 必须本周完成/确认的动作」，让收件人一眼知道要盯什么。
4. **人员时效与改进要求（重点）**：基于 operation_issue.by_handler（各处理人 total/done/overdue/done_rate/overdue_rate），计算各相关人员的工单完成及时率，识别出**完成及时率偏低、超期突出的处理人**，对其**明确提出改进要求**（如：要求压缩 XX 工单处置时长、限期闭环超期项）；若存在多名问题人员，逐一点名与要求。若人员时效整体良好，简要肯定。
...
- ### 2.2 重点推进事项：逐条分析 active 中的重点工作，**必须基于每个对象的 this_week_plan 和 this_week_progress 描述本周计划完成情况**。详写原则：只对有本周计划/本周进展/逾期风险/P0/P1 的事项展开；无实质进展的（this_week_plan 为空且 this_week_progress 为空）用一句话概括即可，不要展开。格式：【标题】（分类，负责人，优先级，进度%，计划完成日）——本周计划完成情况/卡点——下周关键任务。最多详写 6 项，其余合并为「其他 X 项按计划推进」。
