...
// ==================== 后端地址（PMWB 统一托管，原 2525 本地中继已退役）====================
const RELAY_SERVER = 'http://127.0.0.1:8000/api/v1';
...
async function sendDirect() {
  ...
  // 邮件统一由 PMWB 后端 → 统一邮件中心发送，无需本地 SMTP 配置
  const cfg = {};

  // 检测本地服务器是否在线
  const serverOnline = await checkRelayServer();
  if (!serverOnline) {
    showToast('❌ PMWB 后端未启动，请先启动个人工作台系统', 'error');
    return;
  }

  const to = selected.map(r => r.email).filter(Boolean);
  ...
  try {
    ...
    const response = await fetch(`${RELAY_SERVER}/plugins/send`, {
      method: 'POST',
    ...
    if (e.message.includes('Failed to fetch') || e.message.includes('NetworkError')) {
      showToast('❌ 无法连接到 PMWB 后端，请确认已启动个人工作台系统', 'error');
    } else {
...
async function checkRelayServer() {
  try {
    const resp = await fetch(`${RELAY_SERVER}/plugins/health`, { signal: AbortSignal.timeout(3000) });
...
async function loadContacts() {
  // 收件人统一从 PMWB 后端的 sa_info 表加载（原 2525 本地中继已退役）
  try {
    const response = await fetch(`${RELAY_SERVER}/plugins/contacts`, { signal: AbortSignal.timeout(5000) });
    const result = await response.json();
    console.log('[Contacts] PMWB 查询结果:', result);
    if (result.success && Array.isArray(result.contacts)) {
      if (result.contacts.length > 0) {
        window._cachedContacts = result.contacts;
        window._contactsFromDb = true;
        renderContactList(result.contacts);
        renderRecipientOptions(result.contacts);
        return;
      }
      console.warn('[Contacts] PMWB 返回空列表');
    } else if (!result.success) {
      console.warn('[Contacts] PMWB 查询失败:', result.error);
      showToast('⚠️ 从 PMWB 加载收件人失败：' + (result.error || '未知错误'), 'error');
    }
  } catch (e) {
    console.warn('[Contacts] 从 PMWB 加载失败:', e.message);
    showToast('⚠️ 无法连接 PMWB 后端，请确认已启动个人工作台系统', 'error');
  }

  // PMWB 返回空或加载失败，降级到本地存储
...
async function addContact() {
  ...
  // 如果收件人来自 PMWB 后端，同步写入 sa_info 表
  if (window._contactsFromDb) {
    // 先检查同名同系统是否重复
    try {
      const checkResp = await fetch(`${RELAY_SERVER}/plugins/contacts/check?sa_name=${encodeURIComponent(name)}&system_name=${encodeURIComponent(system)}`);
    ...
    // 写入数据库
    try {
      const addResp = await fetch(`${RELAY_SERVER}/plugins/contacts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sa_name: name, system_name: system, email, wechat_nickname: wechatNickname })
      });
...
async function saveEditContact(index) {
  ...
  if (window._contactsFromDb) {
    ...
    try {
      const resp = await fetch(`${RELAY_SERVER}/plugins/contacts`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          old_name: old.name,
...
async function deleteContact(index) {
  ...
  if (window._contactsFromDb) {
    try {
      const resp = await fetch(`${RELAY_SERVER}/plugins/contacts?sa_name=${encodeURIComponent(c.name)}&system_name=${encodeURIComponent(c.system || '')}&email=${encodeURIComponent(c.email)}`, {
        method: 'DELETE'
      });
      const result = await resp.json();
      if (!result.success) {
...
async function importExcelFile(file) {
  try {
    ...
    showToast(`✅ 已导入 ${contacts.length} 个联系人，正在同步...`, 'success');

    // 同步写入 PMWB 后端的 sa_info 表（统一数据源）
    let added = 0;
    let skipped = 0;
    for (const c of contacts) {
      try {
        const resp = await fetch(`${RELAY_SERVER}/plugins/contacts`, {
          ...
          body: JSON.stringify({
            ...
            email: c.email,
            wechat_nickname: c.wechatNickname || ''
          })
        });
        const result = await resp.json();
        if (result.success) {
    ...
    showToast(`✅ PMWB 同步完成：新增 ${added} 个，跳过 ${skipped} 个`, 'success');
...
async function loadSmtpConfig() {
  const { smtpConfig, useMailCenter } = await chrome.storage.local.get(['smtpConfig', 'useMailCenter']);
  ...
  // 统一邮件中心开关
  const useMailCenterEl = document.getElementById('use-mail-center');
  if (useMailCenterEl) useMailCenterEl.checked = useMailCenter !== false;
  // 邮件签名
...
async function saveSmtpConfig() {
  const useMailCenter = document.getElementById('use-mail-center').checked;
  const config = {
  ...
  // 未启用统一邮件中心时，SMTP 配置为必填
  if (!useMailCenter && (!config.host || !config.user || !config.pass)) {
    showToast('未启用统一邮件中心时，请填写 SMTP 服务器、账号和密码/授权码', 'error');
    return;
  }

  // 加密敏感字段后再存储
  if (config.pass) config.pass = await encrypt(config.pass);

  await chrome.storage.local.set({ smtpConfig: config, useMailCenter: useMailCenter });
...
async function testDbConnection() {
  ...
  try {
    const resp = await fetch(`${RELAY_SERVER}/plugins/health`, { signal: AbortSignal.timeout(5000) });
    const data = await resp.json();
    if (data.status === 'ok') {
      resultEl.style.color = '#0f9d58';
      resultEl.textContent = '✅ 已连接到 PMWB 后端，邮件中心运行正常';
    } else {
      resultEl.style.color = '#d93025';
      resultEl.textContent = '❌ PMWB 后端异常：' + (data.error || data.detail || '未知错误');
    }
  } catch (err) {
    resultEl.style.color = '#d93025';
    resultEl.textContent = '❌ 无法连接 PMWB 后端，请先启动个人工作台系统: ' + err.message;
  }
}
...
async function writeToDatabase(emailData) {
  try {
    const resp = await fetch(`${RELAY_SERVER}/plugins/ingest`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function checkServerManually() {
  ...
  const online = await checkRelayServer();
  if (online) {
    resultEl.textContent = '✅ PMWB 后端运行正常 (127.0.0.1:8000)';
    resultEl.style.color = '#0f9d58';
  } else {
    resultEl.textContent = '❌ 未检测到 PMWB 后端，请先启动个人工作台系统';
    resultEl.style.color = '#d93025';
  }
  updateServerStatus();
}
...
async function testSmtpSend() {
  ...
  try {
    const payload = {
      to: [smtpConfig.user].filter(Boolean),
      cc: [],
      subject: '【测试邮件】需求提取插件邮件发送测试',
      body: '如果您收到此邮件，说明 PMWB 后端邮件发送配置正确，插件可以正常发送邮件。\n\n本邮件由统一邮件中心发送。',
      bodyFormat: 'text'
    };
...
function bindEvents() {
  ...
  document.getElementById('use-mail-center').addEventListener('change', async (e) => {
    await chrome.storage.local.set({ useMailCenter: e.target.checked });
  });
