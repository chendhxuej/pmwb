...
> ⚠️ **本文档已归档（2026-08-04 标注）**：其中的 Worktree 并行开发方式**已弃用**。
> 现行协同开发规范以 `docs/COLLABORATIVE_DEV_WORKFLOW.md`（feature 分支 → 审查 → 合版）为准，
> 任务总表见 `.workbuddy/tasks/TASKS.md`。本文档仅保留早期模块拆分与开发计划的历史参考价值。
